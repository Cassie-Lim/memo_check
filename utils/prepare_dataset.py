...
def prepare_test_data(args):
    ...
    if not hasattr(args, 'corruption') or args.corruption == 'original':
        ...
        teset_raw = np.load(args.dataroot +
                            f'/CIFAR-10-C/{args.corruption}.npy')
    ...
    #     path = '/data/yusun/lmy/CIFAR-10c/' + args.corruption + '_' + str(
    #         args.level) + '.npy'
    #     teset.data = np.load(path)
    elif args.corruption == 'cifar_new':
        from utils.cifar_new import CIFAR_New
        print('Test on CIFAR-10.1')
        teset = CIFAR_New(root=args.dataroot + 'CIFAR-10.1/datasets/',
                          transform=te_transforms)
    else:
